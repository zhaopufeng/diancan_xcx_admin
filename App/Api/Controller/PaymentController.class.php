...
class PaymentController extends PublicController {
	...
	public function buy_cart(){
		...
		//获取运费
		$yunfei = $post->where('id=1')->getField('post');
		$yunfei = (float)$yunfei;
		//计算总价
	    foreach($id as $ks => $vs){
        ...
        echo json_encode(array('status'=>1,'price'=>floatval($price),'pro'=>$pro,'adds'=>$add,'addemt'=>$addemt,'yun'=>$yunfei));
		exit();
	}
    ...
    public function payment(){
		...
		//生成订单
		  try {
			...
			//获取运费
			$yunPrice = $post->where('id=1')->getField('post');
			$yunPrice = (float)$yunPrice;
			$data['post'] = $yunPrice;
			//$data['shop_id']=$shop[$ke]['shop_id'];
			$data['uid']=intval($uid);
			$data['price']=floatval($ozprice);
            if(!empty($yunPrice)){
                $data['amount']=floatval($ozprice)+$yunPrice;
			}else{
                $data['amount']=floatval($ozprice);
			}
