...
class OrderController extends PublicController {
	...
	public function index(){
        ...
        $order = $orders->where($condition)->order('id desc')->field('id,order_sn,pay_sn,status,price,type,product_num,addtime,amount')->select();
		foreach ($order as $n=>$v){
			...
			$order[$n]['time'] = date('Y-m-d H:i:s',$v['addtime']);
		}
	...
	public function order_details(){
		...
		$order_info = $orders->where('id='.intval($order_id.' AND del=0'))->field('id,order_sn,shop_id,status,addtime,price,type,post,tel,receiver,address_xq,remark,amount')->find();
		if (!$order_info) {
		...
		$order_info['yunfei'] = 0;
		if ($order_info['post']) {
			$order_info['yunfei'] = $order_info['post'];
		}
